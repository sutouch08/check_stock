...
function insert_to_temp( $id_order, $id_pa, $qty, $id_wh, $id_zone, $status, $id_emp)
{
	...
	while( $qty > 0 )
	{
		$qs = dbQuery("INSERT INTO tbl_temp ( id_order, id_product_attribute, qty, id_warehouse, id_zone, status, id_employee ) VALUES ( ".$id_order.", ".$id_pa.", 1, ".$id_wh.", ".$id_zone.", ".$status.", ".$id_emp." )");
		$qty--;
		if( ! $qs ){ $sc = FALSE; }
	}
	if( $sc === TRUE )
